...
export default async function NonprofitCategorizePage() {
  ...
  if (
    profile?.role !== "admin" &&
    !(profile?.allowed_books ?? []).includes("nonprofit")
  )
    redirect("/nonprofit");

  const admin = await createServiceClient();
  const completeness = await categorizationCompleteness(admin, "nonprofit");
